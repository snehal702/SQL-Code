...
select a.film_id, name,count(a.film_id),  b.category_id from film a join film_category b on a.film_id = b.film_id
...
select distinct(l.name), release_year, count(l.language_id) from language l left join film f on f.language_id = l.language_id where release_year between 2001 and 2010 group by name;
select * from film;
...
select release_year, count(film_id) from film group by release_year order by count(film_id) desc ;
use lco_films;
select * from film;
...
select * from film where title='No Time to Die';film_actorfilm_category
...
#Q22) The entire category of the movie WEST LION was wrongly inserted.
 ...
 select count(actor_id) from film_actor inner join film on film.film_id =  film_actor.film_id where release_year=2017;
...
select name, count(name), release_year from language left join film on film.language_id =language.language_id group by release_year;
select * from language;
...
('Nancy','Perry','1988-05-16','K59042656E','nancy@gmail.com');
...
('2020-08-25', '2020-08-30',(select id from customer where first_name ='Nancy'), (select id from vehicle_type where name ='Economy SUV'),
...
select * from rental;
select c.driver_license_number, ri.rental_id, c.id,  count(ri.rental_id) as num_rental_licenses from 
...
select * from rental_has_insurance; select sum(cost) from insurance;
...
select * from orders; select * from orderdetails; select * from products;products
...
#Q10) Fetch the details of salesmen/emp dealing with customers whose orders are resolved.
...
select count(o.customer_id) from  orders o left join customers c on c.customer_id = o.customer_id where country = 'Finland';
...
select count(cproducts.customer_id) from orders o left join customers c on c.customer_id = o.customer_id where country ='USA';
...
select date_format(order_date, '%M') from orders; -- reqd format of the date
select dayname(order_date) from orders; -- weekday name
select order_date, adddate(order_date, interval 5 day) from orders;
...
(case when CountryName ='India' then 0 else 1 end),CountryName asc; -- making india as 1st record (0) and the rest starting from row 1 & ,CountryName is used to 
...
select substr(CountryName,1,3) from test;
...
select date_add('2022-05-29', interval -8 day), adddate('2022-05-29', interval -8 day);
select id, dt, avg(speed) from wifi
where dt>= date_add('2022-05-29', interval -7 day) -- this gives records only for last 7 days and then do avg calc and groupby on these records
